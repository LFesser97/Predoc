...
This files contains functions for computing various linguistic drifts between corpora.
"""
...
class CorporaComparison:
    ...
    def return_struct_drift(self) -> float:
        """
        Return the average structural drift between corpus 1 and the texts in corpus 2.

        Returns
        -------
        struct_drift : The average structural drift between corpus 1 and the texts in corpus 2.
        """
        assert self.pos_5gram_model != None, "You need to set the pos_5gram_model attribute of the CorporaComparison object before you can compute the structural drift."

        # compute the structural drift for each text in corpus 2 and append it as an 
        # additional column to corpus 2
        struct_drifts = []
        for i in range(len(self.corpus2)):
            struct_drift = __compute_struct_drift()
            struct_drifts.append(struct_drift)

        self.corpus2['struct_drift'] = struct_drifts

        # compute the average structural drift
        struct_drift = np.mean(struct_drifts)

        return struct_drift
    

    def __compute_struct_drift(self, text: str) -> float:
        """
        Compute the structural drift between corpus 1 and a text in corpus 2,
        where the structural drift is defined as the cross-entropy between the 
        true POS tag sequence for the text and the predictions of a POS 5-gram model
        trained on POS tag sequences in corpus 1, saved in self.pos_5gram_model
        ...
        Returns
        -------
        struct_drift : The structural drift between corpus 1 and the text in corpus 2.
        """
        # starting from the fourth word, compute the cross-entropy between the true POS tag sequence for the text and the predictions of a POS 5-gram model trained on POS tag sequences in corpus 1
        struct_drift = 0
        for i in range(3, len(text)):
            true_pos_tag_sequence = text[i-3:i]
            predicted_pos_tag_sequence = self.pos_5gram_model.predict(text[i-3:i])
            struct_drift += true_pos_tag_sequence * np.log(predicted_pos_tag_sequence)
